...
def test_gate_registry_covers_claim_ledger_exactly():
    ...
    gates = json.loads(
        (REPO_ROOT / "data/registry/claim_gate_registry.json").read_text(
            encoding="ascii"
        )
    )["gates"]
...
def test_gate_results_retain_structural_counterexamples():
    results = {
        result["claim_id"]: result for result in execute_claim_gates()["results"]
    }
    assert (
        results["octonion_normed_division_boundary"]["metrics"][
            "zero_divisor_product_norm_squared"
        ]
        == 0.0
    )
    assert (
        results["external_fourier_quotient_map"]["metrics"][
            "maximum_rejected_exact_triad_count"
        ]
        == 0
    )
    assert results["e9_e10_e11_classification"]["metrics"]["E9"]["determinant"] == 0
    assert results["higher_cayley_dickson_physical_mapping"]["metrics"][
        "present_experimental_fields"
    ] == []
    assert results["e10_e11_supergravity_scope"]["metrics"][
        "e11_is_conjectural"
    ] is True
    assert len(
        results["fractional_and_negative_dimension_scope"]["metrics"][
            "implemented_estimators"
        ]
    ) == 4
    assert results["kac_moody_fluid_transfer"]["metrics"][
        "not_implemented_method_count"
    ] >= 4
    assert results["gravitoelectromagnetism_scope"]["metrics"][
        "paper_rejects_new_coupling"
    ] is True
