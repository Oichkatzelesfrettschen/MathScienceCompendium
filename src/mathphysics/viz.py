...
class Visualizer:
    ...
    def plot_roots_2d(self, roots: np.ndarray, title: str = "Root System", 
                      weights: Optional[np.ndarray] = None) -> plt.Figure:
        ...
        scatter = ax.scatter(roots_2d[:, 0], roots_2d[:, 1], 
                            c=c_vals, 
                            cmap='viridis', s=100, alpha=0.8, edgecolors='white', linewidth=0.5)
    ...
    def plot_time_series_with_stats(self, x: np.ndarray, y: np.ndarray, 
        ...
        if y_err is not None:
            ax.fill_between(x, y - y_err, y + y_err, color=self.colors[0], alpha=0.2, label='1σ Confidence')
        ...
        if np.any(significant):
            ax.scatter(x[significant], y[significant], color=self.colors[1], s=50, 
                      marker='*', label='Significant Deviation (p < 0.05)', zorder=5)
    ...
    def plot_interactive_roots(self, roots: np.ndarray, name: str = "roots"):
        ...
        try:
            ...
            fig = px.scatter_3d(df, x='PC1', y='PC2', z='PC3', color='Norm',
    ...
    def plot_persistence_barcode(self, persistence: np.ndarray, title: str = "Persistence Barcode") -> plt.Figure:
        ...
        legend_elements = [Line2D([0], [0], color=self.colors[0], lw=2, label='H0 (Components)'),
...
def create_all_plots():
    ...
    fig = viz.plot_time_series_with_stats(x, y, y_err, 
