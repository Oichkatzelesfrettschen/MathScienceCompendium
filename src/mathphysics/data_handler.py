...
class DataHandler:
    ...
    def save_to_parquet(data_dict: Dict[str, Any], filename: str) -> None:
        ...
        processed_data = {k: [v.tolist() if isinstance(v, (np.ndarray, jnp.ndarray if 'jnp' in globals() else np.ndarray)) else v] 
    ...
    @staticmethod
    def save_simulation_state(density: np.ndarray, velocity: np.ndarray, iteration: int, filename_prefix: str = "lbm_state") -> None:
...
class SimulationDataHandler:
    ...
    def load_state(self, filename_prefix: str, iteration: int) -> Optional[np.ndarray]:
