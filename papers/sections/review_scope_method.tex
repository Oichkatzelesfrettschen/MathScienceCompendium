...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash\bfseries}p{0.20\textwidth}Y}
    ...
    \midrule
    Definition & An object, domain, and declared convention. \\
    ...
    Falsified within scope & A counterexample or rejecting result for the stated claim and regime. \\
...
\paragraph{Follow the specific record.}
Read a disposition in the \evidencerecord{unified_framework_claims.json}{canonical
claim ledger}, then follow its named hypothesis in the
\evidencerecord{hypothesis_registry.json}{hypothesis registry}. The
\evidencerecord{independent_reproduction_registry.json}{reproduction registry}
records repeated computational decisions. A failed positive decision rule can
leave component effects unresolved; read the exact rejection contract.
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.36\textwidth}
  ...
  \midrule
  \endhead
  \input{generated/review_claim_status_rows.tex}
\end{longtable}
\normalsize
