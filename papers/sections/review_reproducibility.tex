...
The retained clean run executes all 540 production cells, 48 refinement cells,
and declared controls with zero resumed cells. Its normalized selector,
production, refinement, and control JSON records match the primary outcomes;
the three raw-array archives are byte-identical. The verification report binds
the exact source tree, image digest, environment, preregistrations, results,
archives, run log, and verification harness before either result is admitted.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{p{0.34\textwidth}X}
    ...
    \path{data/reproduction/} & Cache-isolated clean-container results, raw arrays, environment record, and logs. \\
...
\begin{itemize}
  ...
  \item the 540-run freely decaying beta-plane matrix does not support its
    registered transient-zonalization effect within the tested domain and does
    not license inference outside it; and
