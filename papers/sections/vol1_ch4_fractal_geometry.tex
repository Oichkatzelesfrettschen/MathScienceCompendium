...
Results from \texttt{src/mathphysics/fractal\_analysis.py}:
...
The box-counting implementation produces estimates for declared finite-resolution inputs. Agreement on self-similar fixtures tests the implementation but does not replace a dimension proof. See \texttt{src/mathphysics/fractal\_analysis.py} and \cite{falconer2004fractal}.
\end{factcheck}
...
The finite-resolution estimator in \texttt{src/mathphysics/fractal\_analysis.py} gives $\dim_B(\partial M) \approx 1.998$ for the declared sampling procedure; this does not prove the conjectured limiting value.
