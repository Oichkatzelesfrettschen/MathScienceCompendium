...
\fancyfoot[L]{\small\href{https://github.com/Oichkatzelesfrettschen/MathScienceCompendium/blob/main/docs/learning/README.md}{Learning routes and companion directory}}
\newcommand{\evidencerecord}[2]{\href{https://github.com/Oichkatzelesfrettschen/MathScienceCompendium/blob/main/data/registry/#1}{#2}}
...
\chapter*{Choose a claim, then follow its support}
\addcontentsline{toc}{chapter}{Choose a claim, then follow its support}
Begin with Scope, Questions, and Review Method to distinguish mathematical
arguments, computational observations, and physical measurements. The audit
of physical claims identifies the extra inference behind each proposed map.
The hypothesis registry and conclusions record what survives the declared tests.
Use the reproducibility appendix when you choose to inspect or repeat a result.

The companion bridge book, \emph{From Precalculus to Mathematical Physics},
provides solved lessons in proof, probability, algebra, and numerical reasoning.
Return to a specific lesson when an operation is unfamiliar. The independent
\emph{Precalculus Through Problems of the Past} supplies the earlier foundations.
The album's \emph{index.html} and \emph{navigation.pdf} name exact sections and
physical PDF pages in the separate books. The running footer opens the online
learning directory; the visible book titles also work with each PDF's outline.

A definition fixes a convention. An established result supplies assumptions and
a proof or source. A computational observation names finite inputs and output.
An open hypothesis names its possible rejection. A falsified claim identifies
the tested statement and its scope. These labels remain readable without color.
The evidence guide in the learning directory demonstrates two different negative
decisions and links their canonical records.
...
\input{sections/review_hypothesis_registry}
