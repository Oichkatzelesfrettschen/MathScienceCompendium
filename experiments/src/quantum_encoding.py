...
from qiskit.circuit.library import QFT, MCMT, MCXGate, UnitaryGate
...
class BinaryEncoder(QuantumEncoder):
    ...
    def float_to_binary(self, value: float, signed: bool = True) -> str:
        """Convert float to fixed-point binary representation.
        ...
        if signed:
            max_val = 2**(self.precision - 1) - 1
            min_val = -2**(self.precision - 1)
            scaled = int(value * max_val)
            scaled = max(min_val, min(max_val, scaled))
            # Two's complement for negative
            if scaled < 0:
                scaled = 2**self.precision + scaled
        else:
            max_val = 2**self.precision - 1
            scaled = int(abs(value) * max_val)
            scaled = min(max_val, scaled)

        return self.int_to_binary_string(scaled, self.precision)

    def binary_to_float(self, binary_str: str, signed: bool = True) -> float:
        """Convert binary string back to float value.

        Args:
            binary_str: Binary representation
            signed: Whether value is signed

        Returns:
            Decoded float value
        """
        int_val = self.binary_string_to_int(binary_str)

        if signed:
            # Check for two's complement negative
            if int_val >= 2**(self.precision - 1):
                int_val = int_val - 2**self.precision
            max_val = 2**(self.precision - 1) - 1
...
def demonstrate_encodings():
    ...
    print(f"E7 Index Encoding Circuit:")
    ...
    print(f"E8 Index Encoding Circuit:")
    ...
    print(f"Single Root Amplitude Encoding:")
    ...
    print(f"Amplitude Encoding Validation:")
