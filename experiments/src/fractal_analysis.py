...
This module provides comprehensive tools for analyzing fractal dimensions
using multiple methods including Hausdorff dimension, box-counting dimension,
and correlation dimension. It also includes fractal generation capabilities
for various classic fractals.
"""
...
class FractalGenerator:
    ...
    def lorenz_attractor(num_points: int = 10000,
        ...
        return np.array(points[1000:])  # Skip transient
    ...
    def henon_map(num_points: int = 10000,
        ...
        return np.array(points[100:])  # Skip transient
...
class FractalDimensionCalculator:
    ...
    def box_counting_dimension(points: np.ndarray,
        ...
        if min_scale is None:
            min_scale = 1.0 / (2 ** 10)  # Very fine scale
        if max_scale is None:
            max_scale = 0.5  # Half the domain
        ...
        for scale in scales:
            ...
            if grid_points.shape[1] == 2:
                unique_boxes = len(np.unique(grid_points[:, 0] * grid_size + grid_points[:, 1]))
            elif grid_points.shape[1] == 3:
                unique_boxes = len(np.unique(
                ...
                unique_tuples = set(map(tuple, grid_points))
                unique_boxes = len(unique_tuples)
        ...
        if len(log_scales) < 3:
            raise ValueError("Insufficient valid data points for dimension calculation")

        # Perform linear regression
        slope, intercept, r_value, p_value, std_err = linregress(log_scales, log_counts)

        # Box-counting dimension is negative of the slope
        dimension = -slope
...
def analyze_known_fractals(output_dir: Optional[Path] = None) -> Dict[str, Any]:
    ...
    print(f"Typical dimension: ~2.06")
    ...
    print(f"Typical dimension: ~1.26")
    ...
    if len(mandel_boundary) > 100:
        ...
        print(f"Theoretical dimension: 2.0 (conjectured)")
...
if __name__ == "__main__":
    ...
    fractal_results = analyze_known_fractals()
